...
def extract(doc):
	...
	if (len(candidates) == 0):
		print('No candidates found in', doc.title)
		return
...
def load_doc(doc):
	...
	for page in range(pages):
	    ...
	    cmd = ["python2", "./pdf2txt/tools/pdf2txt.py", "-o", "./temp/output"+str(doc.OFFSET+page)+".txt", "-p", str(doc.OFFSET+page),"-t", "text", doc.path]
...
def table_extract(doc, pages_to_extract):
	...
	for page in pages_to_extract:
	    for area in areas:
	        ...
	        try:
	            table = tabula.read_pdf(doc.path, pages=page, area = area)
